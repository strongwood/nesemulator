...
namespace NESEmulator
{
    public partial class MainWindow : Window
    {
        ...
        private void OpenKeyBindingWindowForPlayer(int playerIndex)
        {
            ...
            KeyBindingWindow keyBindingWindow = new KeyBindingWindow(currentMapping, playerIndex)
            {
                Owner = this
            };
            ...
            if (keyBindingWindow.ShowDialog() == true)
            {
                if (playerIndex == 0)
                {
                    player1KeyboardMapping = keyBindingWindow.ResultMapping;
                    player1KeyboardMapping.Save(GetKeyboardMappingPath(0));
                }
                else
                {
                    player2KeyboardMapping = keyBindingWindow.ResultMapping;
                    player2KeyboardMapping.Save(GetKeyboardMappingPath(1));
                }

                StatusText.Text = $"按键已更新: {GetControlSummary()}";
            }
        }
        ...
        private void MainWindow_KeyDown(object sender, KeyEventArgs e)
        {
            if (nes != null)
            {
                ...
                {
                    nes.SetControllerButton(0, player1Button, true);
                    handled = true;
                }

                if (player2KeyboardMapping.TryGetButton(key, out ControllerButton player2Button))
                {
                    nes.SetControllerButton(1, player2Button, true);
                    handled = true;
                }
        ...
        private void MainWindow_KeyUp(object sender, KeyEventArgs e)
        {
            if (nes != null)
            {
                ...
                if (player1KeyboardMapping.TryGetButton(key, out ControllerButton player1Button))
                {
                    nes.SetControllerButton(0, player1Button, false);
                    handled = true;
                }

                if (player2KeyboardMapping.TryGetButton(key, out ControllerButton player2Button))
                {
                    nes.SetControllerButton(1, player2Button, false);
                    handled = true;
                }
        ...
        {
            return $"P1 A {player1KeyboardMapping.GetKey(ControllerButton.A)} B {player1KeyboardMapping.GetKey(ControllerButton.B)} | P2 A {player2KeyboardMapping.GetKey(ControllerButton.A)} B {player2KeyboardMapping.GetKey(ControllerButton.B)}";
        }
    }
}
