...
{
    /// <summary>
    /// Mapper 003 (CNROM) - 简单的CHR银行切换映射器
    /// 支持的游戏：Solomon's Key, Arkanoid, Gradius等
    /// </summary>
    public class Mapper003 : ICartridge
    {
        ...
        public Mapper003(INESHeader header, byte[] romData)
        {
            this.header = header;
        ...
        public void WritePRG(ushort address, byte value)
        {
            ...
            {
                // CHR银行选择
                chrBankSelect = value & 0x03; // 只使用低2位
            }
        }
        ...
        public byte ReadCHR(ushort address)
        {
            ...
            // CHR银行切换 (8KB银行)
            int bankOffset = chrBankSelect * 8192;
            int chrAddress = bankOffset + address;
